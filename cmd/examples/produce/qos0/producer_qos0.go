...
func main() {
	...
	pulsarTopic := fmt.Sprintf("persistent://public/default/%s", mqttTopic)
	ops := mqtt.NewClientOptions().SetUsername("username").SetClientID("foo").AddBroker(fmt.Sprintf("tcp://localhost:%d", 1883))
